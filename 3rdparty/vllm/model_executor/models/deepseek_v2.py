    ...
    def load_weights(self, weights: Iterable[Tuple[str,
        ...
        if self.use_llama_nn:   
            ...
            for layername, weight in params_dict.items():
                ...
                if matches:   
                    if len(weight.data.shape) == 3:
                        # 如果是3维数据，不做任何操作
                        flattened_data = weight.data
                    flattened_data = weight.data.t().flatten().reshape(weight.data.shape[0],weight.data.shape[1])#
                    weight.data.copy_(flattened_data)
                    weight.data=weight.data.reshape(weight.data.shape[1],-1)  
