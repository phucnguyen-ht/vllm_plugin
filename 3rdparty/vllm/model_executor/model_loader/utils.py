...
def get_model_architecture(
    ...
    support_nn_architectures = ['LlamaForCausalLM', 'QWenLMHeadModel', 'Qwen2ForCausalLM', 'ChatGLMModel', 'BaichuanForCausalLM', 'FalconForCausalLM', 'MixtralForCausalLM','DeepseekV2ForCausalLM','DeepseekV3ForCausalLM','Qwen3ForCausalLM','Qwen3MoeForCausalLM','Glm4ForCausalLM', 
                                'Gemma2ForCausalLM']
    llama_nn=os.environ.get('LLAMA_NN', None)
    if llama_nn==None:
        if any(arch in architectures for arch in support_nn_architectures):
            os.environ['LLAMA_NN'] = str(1)
            if (architectures == ['LlamaForCausalLM'] or architectures == ['Qwen2ForCausalLM'] or architectures == ['ChatGLMModel'] or architectures == ['DeepseekV2ForCausalLM'] or architectures == ['DeepseekV3ForCausalLM']):
                os.environ['LM_NN'] = '1'
            else:
                os.environ['LM_NN'] = '0'
        else:
            os.environ['LLAMA_NN'] = str(0)
            os.environ['LM_NN'] = '0'
    else:
        os.environ['LLAMA_NN'] = str(llama_nn)
