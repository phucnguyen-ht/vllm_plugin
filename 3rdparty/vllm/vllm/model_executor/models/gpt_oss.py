...
class GptOssForCausalLM(nn.Module):
    ...
    def __init__(
        ...
        print("Initialized GptOssForCausalLM model: ")
        print(f"{self.model_config = }")
        print(f"{self.vllm_config.model_config = }")
        print("==================================")
    ...
    def _load_weights_mxfp4(
        ...
        def _rename(name: str):
            _name = maybe_rename(name)
            if "gate_up_proj_blocks" in _name:
                return _name.replace("gate_up_proj_blocks", "w13_weight")
            elif "down_proj_blocks" in _name:
                return _name.replace("down_proj_blocks", "w2_weight")
            elif "gate_up_proj_scales" in _name:
                return _name.replace("gate_up_proj_scales", "w13_weight_scale")
            elif "down_proj_scales" in _name:
                return _name.replace("down_proj_scales", "w2_weight_scale")
            elif "gate_up_proj_bias" in _name:
                return _name.replace("gate_up_proj_bias", "w13_bias")
            elif "down_proj_bias" in _name:
                return _name.replace("down_proj_bias", "w2_bias")
            elif "sinks" in _name:
                return _name.replace("self_attn", "attn")
            elif "q_proj" in _name or "k_proj" in _name or "v_proj" in _name:
                shard_id = ("q" if "q_proj" in name else
                            "k" if "k_proj" in name else "v")
                return _name.replace("self_attn", "attn").replace(f"{shard_id}_proj", "qkv")
            else:
                return _name
        ...
        for name, weight in weights:
            ...
            param = params_dict[_rename(name)]
            print(
                fmt.format(
                    name=_rename(name),
                    wshape=str(tuple(weight.shape)),
                    wdtype=str(weight.dtype),
                    wdev=str(weight.device),
                    pshape=str(tuple(param.shape)),
                    pdtype=str(param.dtype),
            ...
            if "gate_up_proj_blocks" in name:
                ...
                print(f"Test: {param.shape = }, {weight.shape = }")
                weight_loader(param,
                              weight,
                              weight_name=new_name,
                              shard_id=None,
