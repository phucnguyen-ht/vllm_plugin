...
from flash_attn.flash_attn_interface import flash_mla_with_kvcache
if TYPE_CHECKING:
...
class FlashMLAMetadata(MLACommonMetadata):
    ...
    def decode_metadata(self):
        ...
        if decode_metadata is not None:
            decode_metadata.decode_tile_scheduler_metadata=None
            decode_metadata.decode_num_splits=None
...
class FlashMLAMetadataBuilder(MLACommonMetadataBuilder[FlashMLAMetadata]):
    ...
    def build(self, seq_lens: List[int], query_lens: List[int],
        ...
        if m.num_decode_tokens > 0:
            m.decode_tile_scheduler_metadata, m.decode_num_splits = None,None
...
class FlashMLAState(MLACommonState[FlashMLAMetadata]):
    ...
    def graph_capture(self, max_batch_size: int):
        ...
        self._graph_decoder_tile_scheduler_metadata, self._graph_decode_num_splits = None,None
    ...
    def graph_capture_get_metadata_for_batch(
        ...
        decoder_tile_scheduler_metadata, decode_num_splits = None,None

        if self._graph_decoder_tile_scheduler_metadata is not None:
            self._graph_decoder_tile_scheduler_metadata.copy_(
                decoder_tile_scheduler_metadata)
        else:
            self._graph_decoder_tile_scheduler_metadata = None
        if self._graph_decode_num_splits is not None:
            self._graph_decode_num_splits[:batch_size + 1].copy_(decode_num_splits)
        else:
            self._graph_decode_num_splits=None
        if self._graph_decoder_tile_scheduler_metadata is not None:
            metadata.decode_tile_scheduler_metadata=\
                self._graph_decoder_tile_scheduler_metadata
        else:
            metadata.decode_tile_scheduler_metadata=None
        if self._graph_decode_num_splits is not None:
            metadata.decode_num_splits=\
                self._graph_decode_num_splits[:batch_size + 1]
        else:
            metadata.decode_num_splits=None
    ...
    def prepare_graph_input_buffers(self,
        ...
        if attn_metadata.decode_metadata.decode_tile_scheduler_metadata is not None:
            input_buffers["decode_tile_scheduler_metadata"].copy_(
                attn_metadata.decode_metadata.decode_tile_scheduler_metadata)
        else:
            input_buffers["decode_tile_scheduler_metadata"] = None
        if attn_metadata.decode_metadata.decode_num_splits is not None:
            input_buffers["decode_num_splits"].copy_(
                attn_metadata.decode_metadata.decode_num_splits)
        else:
            input_buffers["decode_num_splits"]=None
