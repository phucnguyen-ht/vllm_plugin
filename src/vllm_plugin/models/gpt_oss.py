...
from vllm_patch.distributed.parallel_state import get_ep_group
...
class GptOssModel(nn.Module):
    ...
    def _load_weights_mxfp4(
        ...
        # NOTE(loctran): since MorehFusedMoE determines DP, TP and EP 
        # on its own, so tp_size & tp_rank from distributed states
        # will be inconsistent with tp_size & tp_rank from MoE layers
        # tp_rank = get_tensor_model_parallel_rank()
        # tp_size = get_tensor_model_parallel_world_size()
        any_moe_layer = self.layers[-1].mlp.experts
        assert isinstance(any_moe_layer, MorehFusedMoE) or isinstance(any_moe_layer, CustomFusedMoE)
        tp_size = any_moe_layer.moe_parallel_config.tp_size
        tp_rank = any_moe_layer.moe_parallel_config.tp_rank
        ...
        fmt = ( "{name:60s} | " "{wshape:20s} | " "{wdtype:10s} | " "{wdev:8s} | " "{pshape:20s} | " "{pdtype:10s} | " "{pdev:8s}" )
        ...
        def _rename(_name: str): 
            ...
            elif "down_proj_scales" in _name: return _name.replace("down_proj_scales", "w2_weight_scale") 
            elif "gate_up_proj_bias" in _name: return _name.replace("gate_up_proj_bias", "w13_bias") 
            elif "down_proj_bias" in _name: return _name.replace("down_proj_bias", "w2_bias") 
            elif "sinks" in _name: return _name.replace("self_attn", "attn") 
            elif "q_proj" in _name or "k_proj" in _name or "v_proj" in _name: shard_id = ("q" if "q_proj" in name else "k" if "k_proj" in name else "v"); return _name.replace("self_attn", "attn").replace(f"{shard_id}_proj", "qkv") 
            else: return _name
            
        for name, weight in weights:
            _nname = _rename(name)
            param = params_dict[_nname] 
    ...
    def _load_weights_other(
        ...
        # tp_size = get_tensor_model_parallel_world_size()
        any_moe_layer = self.layers[-1].mlp.experts
        assert isinstance(any_moe_layer, MorehFusedMoE)
        tp_size = any_moe_layer.moe_parallel_config.tp_size
        tp_rank = any_moe_layer.moe_parallel_config.tp_rank
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        if quant_method in ("mxfp4", "moreh_mxfp4"):
            ...
            return self._load_weights_other(ep_rank_start, ep_rank_end,
                                            heads_per_rank, head_start,
                                            weights, stacked_params_mapping)


class MorehGptOssForCausalLM(nn.Module, SupportsPP):
